"""Seed only the explicit disposable Money Scenario A (setup/empty state)
browser database.
...
bare-Account creation and truthful empty Accounts/Transactions/Bills/Cash
Flow states, not a scripted "looks ready" fixture.
"""
...
with app.app_context():
    ...
    db.session.flush()
    user = User(email="money-fresh@example.com", password_hash=generate_password_hash("money-pass-123"), active=True)
    db.session.add(user)
    db.session.flush()
